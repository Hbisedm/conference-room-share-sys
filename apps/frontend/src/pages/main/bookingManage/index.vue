<template>
  <div class="p-20px">
     <Form
          ...
          :colon="false"
      >
          <FormItem label="预定人" name="username">
     ...
     <Table :columns="columns" :dataSource="bookingSearchResult" :pagination=" {
        ...
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'action'">
            <Popconfirm title="通过申请" description="是否通过该预订申请？" @confirm="() => handleApply(record.id)" ok-text="确认" cancel-text="取消">
              <a href="#">通过</a>
            </Popconfirm>
            <br />
            <Popconfirm title="驳回申请" description="是否驳回该预订申请？" @confirm="() => handleReject(record.id)" ok-text="确认" cancel-text="取消">
              <a href="#">驳回</a>
            </Popconfirm>
            <br />
            <Popconfirm title="解除申请" description="是否解除该预订申请？" @confirm="() => handleUnbind(record.id)" ok-text="确认" cancel-text="取消">
              <a href="#">解除</a>
            </Popconfirm>
          </template>
        </template>
